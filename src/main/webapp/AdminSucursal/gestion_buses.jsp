                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <%
                                                        String colorOp = "text-secondary";
                                                        if ("DISPONIBLE".equals(b.getEstadoOperativo()))
                                                            colorOp = "text-success";
                                                        else if ("EN_RUTA".equals(b.getEstadoOperativo()))
                                                            colorOp = "text-primary";
                                                        else if ("EN_MANTENIMIENTO".equals(b.getEstadoOperativo()))
                                                            colorOp = "text-warning";
                                                    %>
                                                    <small class="fw-bold text-secondary"><%= b.getEstadoOperativo()%></small>
                                                </td>
                                                <td class="text muted"><%= b.getKilometrajeActual()%>km</td>
                                                <td>
                                                    <div class="d-flex gap-2">
                                                        ...
                                                        <form action="${pageContext.request.contextPath}/Gestionar_Buses" method="POST" class="m-0">
                                                            ...
                                                            <input type="hidden" name="id_bus" value="<%= b.getIdBus()%>">
                                                            <input type="hidden" name="nuevo_estado_operativo" value="EN_MANTENIMIENTO">
                                                            <button type="submit" class="btn btn-sm btn-outline-warning" onclick="return confirm('¿Enviar bus a mantenimiento? (Dejará de aparecer en la programación de viajes)');" title="Enviar a Mantenimiento">
                                                                <i class="bi bi-tools"></i>
                                                            </button>
                                                        </form>
                                                        <% } else if ("EN_MANTENIMIENTO".equals(b.getEstadoOperativo())) {%>
                                                        <!-- abre el modal para registrar detalles -->
                                                        <button type="button" class="btn btn-sm btn-outline-info" data-bs-toggle="modal" data-bs-target="#modalFinMantenimiento<%= b.getIdBus()%>" title="Registrar Fin de Mantenimiento">
                                                            <i class="bi bi-check-circle-fill"></i>
                                                        </button>
                                                        <% } else if ("EN_RUTA".equals(b.getEstadoOperativo())) { %>
                                                        <button type="button" class="btn btn-sm btn-secondary" disabled title="Bloqueado: El bus está en ruta">
                                                            <i class="bi bi-bus-front"></i>
                                                        </button>
                                                        <% }%>
                                                    </div>

                                                    <!-- Modal Fin Mantenimiento -->
                                                    <div class="modal fade text-start" id="modalFinMantenimiento<%= b.getIdBus()%>" tabindex="-1" aria-hidden="true">
                                                        <div class="modal-dialog">
                                                            <div class="modal-content border-info">
                                                                <div class="modal-header bg-info text-dark">
                                                                    <h5 class="modal-title fw-bold">Reporte de Mantenimiento</h5>
                                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                                                </div>
                                                                <form action="${pageContext.request.contextPath}/Gestionar_Buses" method="POST">
                                                                    <div class="modal-body">
                                                                        <input type="hidden" name="accion" value="fin_mantenimiento">
                                                                        <input type="hidden" name="id_bus" value="<%= b.getIdBus()%>">

                                                                        <div class="mb-3">
                                                                            <label class="form-label fw-bold small text-muted">Fecha del Mantenimiento</label>
                                                                            <input type="date" class="form-control" name="fecha_mantenimiento" value="<%= LocalDate.now()%>" required>
                                                                        </div>
                                                                        <div class="row">
                                                                            <div class="col-6 mb-3">
                                                                                <label class="form-label fw-bold small text-muted">Mano de Obra (Q)</label>
                                                                                <input type="number" step="0.01" min="0" class="form-control" name="monto_mano_obra" required>
                                                                            </div>
                                                                            <div class="col-6 mb-3">
                                                                                <label class="form-label fw-bold small text-muted">Repuestos (Q)</label>
                                                                                <input type="number" step="0.01" min="0" class="form-control" name="monto_repuestos" required>
                                                                            </div>
                                                                        </div>
                                                                        <div class="mb-3">
                                                                            <label class="form-label fw-bold small text-muted">Descripción Técnica</label>
                                                                            <textarea class="form-control" name="descripcion" rows="3" maxlength="260" placeholder="Ej. Cambio de aceite, revisión de frenos y reemplazo de filtro..." required></textarea>
                                                                            <div class="form-text text-end">Máximo 260 caracteres</div>
                                                                        </div>
                                                                    </div>
                                                                    <div class="modal-footer bg-light">
                                                                        <button type="button" class="btn btn-secondary fw-bold" data-bs-dismiss="modal">Cancelar</button>
                                                                        <button type="submit" class="btn btn-info fw-bold text-dark">Guardar y Habilitar Bus</button>
                                                                    </div>
                                                                </form>
                                                            </div>
                                                        </div>
